...
- Doctrine assignment limits were only counting active interceptors. A threat that survived a fuse-roll miss became eligible again under `Disciplined`, so every doctrine could quietly behave like shoot-look-shoot. The simulation now tracks persistent per-threat engagement attempts and removes them only when the threat is destroyed or impacts.
...
- UE automation: thirteen `ProjectAirDefense.BattleSimulation` tests passed.
- UE Monte Carlo: 300 runs passed with average intercept rate `0.861`, average city integrity `81.38`, 3300 spawned threats, and 459 hostile impacts.
