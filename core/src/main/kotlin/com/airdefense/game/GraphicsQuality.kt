...
enum class GraphicsQualityMode {
    ...
    companion object {
        fun from(value: String?): GraphicsQualityMode? = entries.firstOrNull { it.name.equals(value, ignoreCase = true) }
    }
}
...
object GraphicsQualityProfiles {
    ...
    fun resolve(
        ...
        when (effectiveMode(requestedMode, deviceClass)) {
            GraphicsQualityMode.PERFORMANCE -> {
                GraphicsQualityProfile(
            ...
            GraphicsQualityMode.AUTO -> {
                error("AUTO must be resolved before building a profile")
            }
        }
}
